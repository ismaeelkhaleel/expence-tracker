...
export default function HomeScreen({ navigation }) {
  ...
  const fetchMonthData = useCallback(async () => {
    const monthKey = getMonthKey(selectedMonth); // e.g. '2026-08'
    const [monthData, latestData] = await Promise.all([
  ...
  const budgetInfo = useMemo(
    () => calculateBudgetInfo(budgetLimit, netExpense, selectedMonth),
    [budgetLimit, netExpense, selectedMonth],
  );
  ...
  const renderTransactionRow = (item) => {
    ...
    return (
      <View
        key={item.id}
        style={[styles.latestItem, { borderBottomColor: theme.border }]}
      >
        ...
        <Text
          style={[typography.body, { color: amountColor, fontWeight: "bold" }]}
        >
          {isExpense ? "-" : "+"}
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { backgroundColor: theme.surface }]}>
        ...
        >
          ...
          <Text style={[typography.h3, { color: theme.text, marginLeft: 8 }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Card>
          ...
          <Text
            style={[typography.h1, { color: theme.text, marginVertical: 8 }]}
          >
            {formatCurrency(netExpense, settings.currencySymbol)}
          </Text>
        ...
        <Card>
          <View style={styles.rowBetween}>
            ...
            <Text style={[typography.body, { color: theme.textSecondary }]}>
              {formatCurrency(budgetLimit, settings.currencySymbol)}
            </Text>
          </View>
            ...
            <View style={{ marginTop: 16 }}>
              <View style={[styles.rowBetween, { marginBottom: 8 }]}>
                <Text
                  style={[typography.small, { color: theme.textSecondary }]}
                >
                  Used: {formatCurrency(netExpense, settings.currencySymbol)}
                </Text>
                <Text
                  style={[typography.small, { color: theme.textSecondary }]}
                >
                  Left:{" "}
                  {formatCurrency(
                    Math.max(budgetInfo.remaining, 0),
                    settings.currencySymbol,
                  )}
                </Text>
              </View>
              <ProgressBar
                progress={budgetInfo.usagePercentage}
              ...
              >
                ...
                {budgetInfo.isExceeded ? (
                  <Text
                    style={[
                      typography.caption,
                      { color: theme.danger, marginTop: 4, fontWeight: "bold" },
                    ]}
                  >
                    Budget exceeded by{" "}
                    {formatCurrency(
                      Math.abs(budgetInfo.remaining),
                      settings.currencySymbol,
                    )}
                  </Text>
                ) : (
                  <Text
                    style={[
                      typography.caption,
                      { color: theme.success, marginTop: 4 },
                    ]}
                  >
                    You can safely spend{" "}
                    <Text style={{ fontWeight: "bold" }}>
                      {formatCurrency(
                        budgetInfo.recommendedDaily,
                        settings.currencySymbol,
                      )}
                    </Text>{" "}
                    / day
      ...
      >
        ...
        >
          <View style={[styles.dateModal, { backgroundColor: theme.surface }]}>
            <Text
              style={[typography.h3, { color: theme.text, marginBottom: 12 }]}
            >
              Select Month
            </Text>
